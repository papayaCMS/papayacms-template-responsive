...
<!--
  ...
  This file contains rules to handle parts from the content xml that are from a richtext
  editor and considered xhtml.

  To apply them to nodes use:

  <apply-templates mode="richtext"/>
  or
  <apply-templates select="node()" mode="richtext"/>
...
<xsl:template match="comment()" mode="richtext">
  <xsl:comment>
    <xsl:value-of select="."/>
  </xsl:comment>
</xsl:template>
...
<xsl:template match="object[@type='application/x-shockwave-flash']" mode="richtext">
  <xsl:choose>
        ...
        <xsl:choose>
          ...
          <xsl:when test="@version">
            <xsl:attribute name="data-swfobject">
              <xsl:call-template name="json-encode-object">
...
<xsl:template match="script" mode="richtext">
  <script>
    <xsl:copy-of select="@*" />
    <xsl:comment>
