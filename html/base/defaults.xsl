...
<!-- scale viewport depending on display size -->
<xsl:param name="ENABLED_VIEWPORT_SCALE" select="true()" />
<!-- disable user scaling -->
<xsl:param name="ENABLED_VIEWPORT_FIXED" select="false()" />
...
<xsl:template name="page-translations">
  <xsl:if test="count(translations/translation) &gt; 1">
    ...
    <ul class="pageTranslations navigation" title="{papaya-fn:language-text('VIEW_CAPTION_TRANSLATIONS')}">
...
<xsl:template name="html-head">
  ...
  <xsl:if test="$ENABLED_VIEWPORT_SCALE">
    <xsl:choose>
      <xsl:when test="$ENABLED_VIEWPORT_FIXED">
        <meta
          name="viewport"
